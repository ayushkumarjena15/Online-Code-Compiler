...
async function runTests() {
  ...
  if (!ids.javascript) ids.javascript = 'nodejs-head';
  if (!ids.python) ids.python = 'cpython-head';
  if (!ids.java) ids.java = 'openjdk-head';
  if (!ids.cpp) ids.cpp = 'gcc-head';
  if (!ids.c) ids.c = 'gcc-head-c';

  const results = [];
  for (const lang of ALL_LANGUAGES) {
    const compilerId = ids[lang.id];
    if (!compilerId) {
      results.push({ lang: lang.id, pass: false, error: 'No compiler ID' });
      continue;
    }
    try {
      ...
      if (res.data.status !== '0' || res.data.compiler_error || res.data.program_error) {
         results.push({ lang: lang.id, pass: false, compiler: compilerId, error: res.data.compiler_error || res.data.program_error });
      } else {
         results.push({ lang: lang.id, pass: true, compiler: compilerId });
      }
    } catch (err) {
      results.push({ lang: lang.id, pass: false, compiler: compilerId, error: err.message });
    }
  }
